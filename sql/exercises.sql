...
select distinct title, description, rating, length from film
WHERE length>=180;
...
select distinct payment_id, amount, payment_date from payment
...
select * from payment order by payment_id limit 20;
...
where amount > 5 limit 50 offset 50; 
...
left join city
on staff.last_update = city.last_update;
